...
//! mcfa emit-slog-db OUT TERM... # write TERM as a loader-only Slog program (no
//!                               # `include "mcfa.slog"`) for the staged --out-db / -d
//!                               # workflow -- see slog/README.md
//! ```
...
fn main() {
   ...
   match cmd {
      ...
      "emit-slog" | "emit-slog-db" => {
         let out = args.get(2).cloned().unwrap_or_else(|| {
            eprintln!("{cmd} needs an OUT_PATH");
            std::process::exit(1);
         });
         let (ast, desc) = match args.get(3).map(|s| s.as_str()) {
            ...
            other => {
               eprintln!("{cmd} OUT_PATH TERM...  where TERM... = worst N K P | church N | feature");
               if let Some(o) = other {
         ...
         if cmd == "emit-slog-db" {
            emit_slog_db(Path::new(&out), &ast, &desc);
         } else {
      ...
      other => {
         ...
         eprintln!("usage: mcfa [run N K P | bench | emit-souffle DIR N K P | emit-slog OUT TERM... | emit-slog-db OUT TERM... | dump DIR N K P]");
         std::process::exit(1);
      },
   }
}
...
/// Render just the term into a `(top ...)` rule string, without writing
/// anything -- shared by both `emit_slog` (full inline program) and
/// `emit_slog_db` (loader-only program for a staged `--out-db` run).
fn render_top_rule(ast: &scheme_mcfa::Ast) -> (String, u64) {
   // Labels are a fresh pre-order counter, mirroring `to_expr_labeled`'s
   // every-occurrence-distinct identity (mcfa.slog never inspects a label's
   // value, only its distinctness).
   let mut next = 1u64;
...
/// same term with per-occurrence labels), formatted as `.expected` sidecar
/// contents.
fn expected_sizes(ast: &scheme_mcfa::Ast) -> String {
   ...
   rows.iter().map(|(name, n)| format!("{name} {n}\n")).collect()
}

/// Full inline program: `include "mcfa.slog"` plus the `(top ...)` fact,
/// with an `.expected` sidecar of relation cardinalities. Handy for quick
/// one-off checks; a brand-new term recompiles the whole analysis, so
/// prefer `emit_slog_db` for a benchmark sweep over many terms.
fn emit_slog(out: &Path, ast: &scheme_mcfa::Ast, desc: &str) {
   use std::fs;

   let (term, n_labels) = render_top_rule(ast);
   let program = format!(
   ...
   let expected = expected_sizes(ast);
   let expected_path = format!("{}.expected", out.display());
   fs::write(&expected_path, expected).expect("write expected sizes");

   println!("wrote {} ({desc}, {n_labels} labelled nodes) and {expected_path}", out.display());
}

/// Loader-only program for the staged `--out-db` / `-d` workflow: just the
/// syntax type declarations (`mcfa-syntax.slog`, shared with mcfa.slog /
/// mcfa-tuned.slog) plus the `(top ...)` fact -- no `include "mcfa.slog"`,
/// so this compiles to a tiny, cheap plugin regardless of term size. Run
/// once per term with `--out-db NAME`; the analysis files are then run
/// against that saved database with `-d NAME`, and since their rule text
/// never changes across terms, their compiled plugins are a cache hit
/// after the first time they are ever compiled. Still writes the
/// `.expected` sidecar (same cardinalities, used for validation either way).
fn emit_slog_db(out: &Path, ast: &scheme_mcfa::Ast, desc: &str) {
   use std::fs;

   let (term, n_labels) = render_top_rule(ast);
   let program = format!(
      ";; generated by: mcfa emit-slog-db ({desc})\n\
       ;; Loader-only program for the staged --out-db / -d workflow: just the\n\
       ;; syntax type declarations, plus this term as a `(top ...)` fact.\n\
       ;; Run with --out-db NAME, then run mcfa.slog / mcfa-tuned.slog with\n\
       ;; -d NAME (see slog/README.md).\n\
